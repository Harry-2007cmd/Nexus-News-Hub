...
- **🔴 Live Breaking News Ticker** — Scrolling headline ticker with the latest articles
...
- **📖 Article Detail Page** — Full article view with share buttons (Facebook, Twitter, WhatsApp, copy link), related news sidebar, and read-time estimate
...
2. **Set your API key**
   ...
   Open `Frontend/Assets/js/main.js` and `Frontend/Assets/js/categories.js` and replace the API key:
   ```javascript
...
3. **Launch with Live Server**
   ...
   Open the project in VS Code, right-click `Frontend/index.html`, and select **"Open with Live Server"**. The app runs on port `5502` by default (configured in `.vscode/settings.json`).
...
> ⚠️ Keep your API key private. For production, proxy requests through a backend server.
...
| Storage | `localStorage` / `sessionStorage` (client-side auth & preferences) |
...
- Account info panel
- Dark mode, alerts, and newsletter preference toggles
...
All colors and design tokens are defined as CSS custom properties in `main.css`:
...
Each page has its own scoped CSS file that inherits these variables, keeping styles modular and easy to maintain.

---

## 🔐 Authentication Notes

Authentication is implemented **client-side only** using `localStorage`:

- User accounts are stored in `localStorage` as a JSON array under the key `"users"`
- Passwords are stored in **plain text** — this is suitable for demo/learning purposes only
- For a production application, implement server-side authentication with hashed passwords (e.g., bcrypt) and session tokens
...
Built with ❤️ for learning and practice. Feel free to use this project as a reference or starting point for your own news application.
